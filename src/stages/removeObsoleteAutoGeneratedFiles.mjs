...
export default async function(fourtune_session) {
	let deleted_entries = []
	const autogenerated_files = fourtune_session.files_to_autogenerate.map(file => file.relative_path)
	const auto_dir_entries = await scandir(path.join(fourtune_session.project.root, "src", "auto"))
	...
	for (const {type, relative_path} of auto_dir_entries) {
		...
		if (!autogenerated_files.includes(`src/auto/${relative_path}`)) {
			await remove(path.join(fourtune_session.project.root, "src", "auto", relative_path))
	...
	for (const directory_on_disk of directories_on_disk) {
		...
		if (!is_used) {
			await remove(path.join(fourtune_session.project.root, "src", "auto", directory_on_disk))

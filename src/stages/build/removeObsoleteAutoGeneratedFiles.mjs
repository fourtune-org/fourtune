...
export default async function(fourtune_session) {
	...
	const auto_folder_entries = await scandir(
		path.join(fourtune_session.project.root, "auto"),
		{
			allow_missing_dir: true
		}
	)

	const obsolete_files = auto_folder_entries.filter(({type, relative_path}) => {
	...
	const obsolete_directories = auto_folder_entries.filter(({type, relative_path}) => {
